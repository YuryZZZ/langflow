    ...
    def update_build_config(self, build_config: dotdict, _field_value: Any, field_name: str | None = None):
        if field_name in {"model_name", "tool_model_enabled", "base_url", "api_key"}:
            try:
                ...
                selected_model_id = build_config["model_name"]["value"]
                selected_model = next((model for model in self.all_models if model.id == selected_model_id), None)

                if selected_model and hasattr(selected_model, 'supports_detailed_thinking') and selected_model.supports_detailed_thinking:
